...
@section('content')
    ...
    <div class="list-grid-nav hstack gap-1 mb-3">
        ...
        {{-- <a href="{{ route('getAddUser') }}?return_url={{ route('getManageTrip') }}" id="btn_add_new_data"
                    ...
                    <table id="table_dashboard" class="table align-middle table-nowrap table-hover mb-0">
                        <thead class="table-blue">
                            <tr class="active">
                                ...
                                <th width="auto"><a href="{{ route('getManageTrip') }}" title="Click to sort">Passenger
                                        Name
                                </th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=pickup_point&sorting={{ request()->get('filter_column') == 'pickup_point' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Pickup Point &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=dropoff_point&sorting={{ request()->get('filter_column') == 'dropoff_point' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Drop Point &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=seats_required&sorting={{ request()->get('filter_column') == 'seats_required' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Seats Required &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=requested_date&sorting={{ request()->get('filter_column') == 'requested_date' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Requested Date &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=luggage_count&sorting={{ request()->get('filter_column') == 'luggage_count' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Luggage Count &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=cart_type&sorting={{ request()->get('filter_column') == 'cart_type' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Car Type &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageTrip') }}?filter_column=status&sorting={{ request()->get('filter_column') == 'status' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                            ...
                            @if (!empty($rows) && count($rows))
                                @foreach ($rows as $data)
                                    <tr>
                                        ...
                                        <td>{{ $data->dropoff_point }}</td>
                                        <td>{{ $data->seats_required }}</td>
                                        <td>{{ \Carbon\Carbon::parse($data->requested_date)->format('d-m-Y H:s') }}</td>
                                        ...
                                        <td>
                                            <div class="button_action" style="text-align:right">
                                                <a class="btn btn-sm btn-success btn-edit" title="View Bid Details"
                                                    href="{{ route('getManageBidDetails', $data->id) }}?return_url={{ route('getManageTrip') }}">View
                                                    Bids</a>
                                                <a class="btn btn-sm btn-primary btn-detail" title="Detail Data"
                                                    href="{{ route('getManageTripDetails', $data->id) }}?return_url={{ route('getManageTrip') }}"><i
                                                        class="fa fa-eye"></i></a>
                                                {{-- <a class="btn btn-sm btn-success btn-edit" title="Edit Data"
                                                    href="{{ route('getEditUser', $data->id) }}?return_url={{ route('getManageTrip') }}"><i
