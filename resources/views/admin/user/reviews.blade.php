    ...
    <div class="card">
        <div class="card-header align-items-center d-flex">
            ...
            <div class="box-tools pull-right" style="position: relative;margin-top: -5px;margin-right: -10px">
                ...
                <form method="get" style="display:inline-block;width: 290px;" action="{{ route('getManageUser') }}">
                        ...
                        <thead class="table-blue">
                            <tr class="active">
                                ...
                                <th width="auto"><a
                                        ...
                                        title="Click to sort">Passenger Name &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageUser') }}?filter_column=email&sorting={{ request()->get('filter_column') == 'email' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort">Driver Name &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                ...
                                @foreach ($rows as $data)
                                    <tr>
                                        <td><input type="checkbox" class="checkbox" name="checkbox[]"
                                                value="{{ $data->id }}"></td>
                                        <td>{{ $data->userDetails ? $data->userDetails->name : 'NA' }}</td>
                                        <td>{{ $data->DriverDetails ? $data->DriverDetails->name : 'NA' }}</td>
                                        <td>{{ $data->review }}</td>
                                        <td>{{ $data->rating }}</td>
                                    ...
                                    t.isConfirmed?location.href='{{ route('deleteReview', $data->id) }}':''});">
