    ...
    <div class="card">
        <div class="card-header align-items-center d-flex">
            ...
            <div class="box-tools pull-right" style="position: relative;margin-top: -5px;margin-right: -10px">
                ...
                <form method="get" style="display:inline-block;width: 290px;" action="{{ route('getManageCMS') }}">
                    <div class="input-group">
                        ...
                        <div class="input-group-btn">
                            @if (!empty(request()->get('q')))
                                <button type="button" onclick="location.href='{{ route('getManageCMS') }}'" title="Reset"
                    ...
                    <div class="input-group">
                        <select onchange="$('#form-limit-paging').submit()" name="limit" style="width: 56px;"
                            ...
                            @foreach ($limis as $lmt)
                                <option value="{{ $lmt }}" {{ $lmt == $limit ? 'selected' : '' }}>{{ $lmt }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <table id="table_dashboard" class="table align-middle table-nowrap table-hover mb-0">
                        <thead class="table-blue">
                            <tr class="active">
                                ...
                                <th width="auto"><a
                                        href="{{ route('getManageCMS') }}?filter_column=page_title&sorting={{ request()->get('filter_column') == 'page_title' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort ascending">Page &nbsp; <i class="fa fa-sort"></i></a></th>
                                <th width="auto"><a
                                        href="{{ route('getManageCMS') }}?filter_column=meta_title&sorting={{ request()->get('filter_column') == 'meta_title' && request()->get('sorting') == 'asc' ? 'desc' : 'asc' }}"
                                        title="Click to sort ascending">Meta Title &nbsp; <i class="fa fa-sort"></i></a>
                                </th>
...
@else
    ...
    @endif
                                            ...
                                            <div class="button_action" style="text-align:right">
                                                @if (AdminHelper::isView())
                                                    <a class="btn btn-sm btn-primary btn-detail" title="Detail Data"
                                                        href="{{ route('getManageCMS') }}/detail/{{ $data->id }}?return_url={{ route('getManageCMS') }}"><i
                                                            class="fa fa-eye"></i></a>
                                                @endif
                                                @if (AdminHelper::isUpdate())
                                                    <a class="btn btn-sm btn-success btn-edit" title="Edit Data"
                                                        href="{{ route('getManageCMS') }}/edit/{{ $data->id }}?return_url={{ route('getManageCMS') }}"><i
        ...
        <i class="fa fa-trash"></i>
                            ...
                            @else
                                <tr>
                                    <td colspan="5" style="text-align:center"><i class="fa fa-search"></i> No Data
                                        Avaliable</td>
                                </tr>
                            @endif
                        </tbody>
